...
namespace SchoolApplication.Repositories.Tests
{
    ...
    public class StudentReadRepositoryTests : SchoolApplicationContextInMemory
    {
        ...
        public async Task GetAllShouldReturnValue()
        {
            // Arrange
            var student = TestDataGenerator.Student();
            var deletedStudent = TestDataGenerator.Student(x => x.DeletedAt = DateTimeOffset.UtcNow);

            Context.AddRange(student, deletedStudent);
            ...
            result.Should().ContainSingle()
                .Which.Should().BeEquivalentTo(student);
        }
    }
}